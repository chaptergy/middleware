...
class DistributedLockManagerService(Service):
    ...
    @private
    async def local_reset(self, eject_peer=True):
        await self.middleware.call('dlm.create')
        lockspace_names = list(await self.middleware.call('dlm.kernel.node_lockspaces', self.peernodeID))
        if lockspace_names and eject_peer:
